...
Test how **species richness** and **functional diversity** are related to evolutionary time, diversification rate, productivity, climatic variables and relief heterogeneity.

### Prediction

If **non-equilibrium** mechanisms are relatively more important to generate differences in species numbers between regions, I expect that **diversification rates and evolutionary time are positively correlated with species richness and functional diversity**. On the other hand, if **equilibrium** mechanisms are more important, I expect that **climate variables present, relief heterogeneity and productivity gradients are strongly and positively related to species richness and functional diversity**.
...
### Shape Atlantic Forest
We used the consensual limit proposed by Muylaert et al. 2018
...
### Vertebrate composition of Atlantic Forest

Composition matrix was obtained by overlaying the occurrence extension polygons with the grid, made in the LetsR package (Vilela and Villalobos, 2015). The data was obtained from different datasets available online and the .shp are available in the supplementary material.
...
### Birds
Checking and adjusting database 
```{r, message = FALSE}
...
Checking dataset and phylogeny
```{r, message = FALSE}
...
```{r, message = FALSE}
birds_phy_list <- prune.sample(short.birds.list, birds_full_phy) #638 espécies 
...
## Calculating metrics
### Diversification rate Equal split 
#### Anura
```{r}
# ANFÍBIOS
library(conflicted)
...
### Mean Pairwise Distance - Evolotionary time
We calculated this measurement for each grid cell separately for each vertebrate group.
...
## Preparing the data
### Grouping the data and saving .shp 
```{r}
#Anfíbios
...
#Disparidade
#Aqui entramos com os autovetores da PCoA (trait-space)
