...
# Assemblage age
# full list
full_aa_amphibia <- read.table('/repos/biogeo_vertebrates-main/assemblage_age/age_amphibia_full.txt')#487
...
```{r, Diversification rate equal split}
# Diversification rate by Herodotools - mean each cell
# full list
hero_dr_amphibia <- read.table('/repos/biogeo_vertebrates-main/assemblage_age/metrics_amphibia_full.txt')#487
hero_dr_squamata <- read.table('/repos/biogeo_vertebrates-main/assemblage_age/metrics_squamata_full.txt')#382
hero_dr_birds <- read.table('/repos/biogeo_vertebrates-main/assemblage_age/metrics_birds_full.txt')#695
hero_dr_mammals <- read.table('/repos/biogeo_vertebrates-main/assemblage_age/metrics_mammals_full.txt')#196
```
...
```{r, fdis with gawdis}
# Matriz de distancia usando função gawdis de Bello et al., 2021
anura_traits <- data.frame(dist_ativ, dist_habitat, as.data.frame(bodysize))

gaw.anura <-gawdis(anura_traits,
                   w.type="analytic",
                   groups =c(1, 1, 1,
                             2, 2, 2, 2,
                             3))

cat("traits wieghts", attr(gaw.anura,"weights")) 
# trait1, trait2 e trait3
# circadian activity, habitat, body size

is.euclid(gaw.anura) # FALSE
is.euclid(sqrt(gaw.anura)) # TRUE

sum(as.matrix(is.na(gaw.anura)))#no NAs 
sum(gaw.anura == 0) # 64 comunidades com zero

####
# Wed Mar 31 16:11:06 2021 ------------------------------
# Calculating Functional Diversity metrics
disp.func.amphibia <- fdisp(gaw.anura, as.matrix(harm_list_amphibia))
...
# Wed Mar 31 16:11:06 2021 ------------------------------
# Calculating Functional Diversity metrics
disp.func.amphibia_antigo <- fdisp(trait.dist, as.matrix(harm_list_amphibia))

FDis_metric_amphibia_antigo <- data.frame(id=harm_aa_amphibia$id, FDis=disp.func.amphibia_antigo$FDis)
```

```{r, exploratory}
dados_traits <- inner_join(FDis_metric_amphibia,
           FDis_metric_amphibia_antigo,
           by="id") %>%
  rename(gawdis_fdis = FDis.x,
         gower_ade4 = FDis.y)
  
head(dados_traits)

ggplot(dados_traits, aes(x = scale(gawdis_fdis), 
                  y = scale(gower_ade4))) +
            geom_point() +
            geom_smooth(method = "lm") +
            labs(x = "gawdis_fdis", y = "gower_ade4")

ggplot(dados_traits, aes(x = gawdis_fdis, 
                  y = gower_ade4)) +
            geom_point() +
            geom_smooth(method = "lm") +
            labs(x = "gawdis_fdis", y = "gower_ade4")

dados_traits_rich <- cbind(dados_traits,
                           rich = harm_rich_amphibia$richness)

ggplot(dados_traits_rich, aes(x = scale(rich), 
                  y = scale(gawdis_fdis))) +
            geom_point() +
            geom_smooth(method = "lm") +
            labs(x = "richness", y = "gawdis_fdis")
```
...
```{r, fdis with gawdis}
# Matriz de distancia usando função gawdis de Bello et al., 2021
squamata_traits <- data.frame(dist_ativ_reptile,
                              dist_habitat_reptile,
                              as.data.frame(bodysize.reptile))

gaw.squamata <-gawdis(squamata_traits,
                   w.type="analytic",
                   groups =c(1, 1,
                             2, 2, 2, 2,
                             3))

cat("traits wieghts", attr(gaw.squamata,"weights")) 
# trait1, trait2 e trait3
# circadian activity, habitat, body size

is.euclid(gaw.squamata) # FALSE
is.euclid(sqrt(gaw.squamata)) # TRUE

sum(as.matrix(is.na(gaw.squamata)))#no NAs 
sum(gaw.squamata == 0) # 6 comunidades com zero

####
# Wed Mar 31 16:11:06 2021 ------------------------------
# Calculating Functional Diversity metrics
disp.func.reptile <- fdisp(gaw.squamata, as.matrix(harm_list_squamata))
?fdisp
FDis_metric_reptiles <- data.frame(id=harm_aa_squamata$id,
                                   FDis=disp.func.reptile$FDis)
...
#Calculando FDisp
disp.func.reptile_antigo <- fdisp(trait.dist.reptile, as.matrix(harm_list_squamata))

FDis_metric_reptiles_antigo <- data.frame(
  id=harm_aa_squamata$id,
  FDis=disp.func.reptile_antigo$FDis
  )
...
```{r, fdis with gawdis}
# Matriz de distancia usando função gawdis de Bello et al., 2021
birds_traits <- data.frame(dist_habitat_birds,
                           atividade_bird,
                           bodysize.birds)

gaw.birds <-gawdis(birds_traits %>% select(-PelagicSpecialist),
                   w.type="analytic",
                   groups = c(1,1,1,1,1,1,1,
                              2,
                              3),
                              fuzzy= c(1))

cat("traits wieghts", attr(gaw.birds,"weights")) 

# trait1, trait2 e trait3
# circadian activity, habitat, body size

is.euclid(gaw.birds) # FALSE
is.euclid(sqrt(gaw.birds)) # TRUE

sum(as.matrix(is.na(gaw.birds)))#no NAs 
sum(gaw.birds == 0) # 6 comunidades com zero

####
# Wed Mar 31 16:11:06 2021 ------------------------------
# Calculating Functional Diversity metrics
disp.func.birds <- fdisp(gaw.birds, as.matrix(harm_list_birds))

FDis_metric_birds <- data.frame(id=full_aa_amphibia$id,
                                   FDis=disp.func.birds$FDis)
hist(FDis_metric_birds$FDis)
```
...
disp.func.birds_antigo <- fdisp(sqrt(trait.dist.birds), as.matrix(harm_list_birds)) #tol = 1e-07

FDis_metric_birds_antigo <- data.frame(id=full_aa_amphibia$id,
                                FDis=disp.func.birds_antigo$FDis)

hist(FDis_metric_birds_antigo$FDis) 
```
...
```{r, fdis with gawdis}
# Matriz de distancia usando função gawdis de Bello et al., 2021
mammals_traits <- data.frame(dist_ativ_mammals, 
                             dist_habitat_mammals,
                             bodysize.mammals)

gaw.mammals <-gawdis(mammals_traits,
                   w.type="analytic",
                   groups = c(1,1,1,1,
                              2,2,2,2,
                              3))
attr(gaw.mammals,"weights")
cat("traits wieghts", attr(gaw.mammals,"weights")) 
# trait1, trait2 e trait3
# circadian activity, habitat, body size

is.euclid(gaw.mammals) # FALSE
is.euclid(sqrt(gaw.mammals)) # TRUE

sum(as.matrix(is.na(gaw.mammals)))#no NAs 
sum(gaw.mammals == 0) # 6 comunidades com zero

#
disp.func.mammals <- fdisp(gaw.mammals, as.matrix(harm_list_mammals))

FDis_metric_mammals <- data.frame(id=full_aa_amphibia$id,
                                   FDis=disp.func.mammals$FDis)
hist(disp.func.mammals$FDis)
```
...
disp.func.mammals_antigo <- fdisp(trait.dist.mammals, as.matrix(harm_list_mammals)) #tol = 1e-07

FDis_metric_mammals_antigo <- data.frame(id=harm_aa_amphibia$id, 
                                         FDis=disp.func.mammals_antigo$FDis)
```

###### 1.3.5. Plot trait-space
```{r, with gawdis}
# Melhorando visualização 
pca_anura <- prcomp(gaw.anura, scale. = TRUE)
anura.pca <- autoplot(pca_anura)

pca_rep <- prcomp(gaw.squamata, scale. = TRUE)
squamata.pca <- autoplot(pca_rep)

pca_bird <- prcomp(gaw.birds, scale. = TRUE)
birds.pca <-autoplot(pca_bird)

pca_mammal <- prcomp(gaw.mammals, scale. = TRUE)
...
```{r, with ade4}
# Melhorando visualização 
pca_anura.ade4 <- prcomp(trait.dist, scale. = TRUE)
anura.pca.ade4 <- autoplot(pca_anura.ade4)

pca_rep.ade4 <- prcomp(trait.dist.reptile, scale. = TRUE)
squamata.pca.ade4 <- autoplot(pca_rep.ade4)

pca_bird.ade4 <- prcomp(trait.dist.birds, scale. = TRUE)
birds.pca.ade4 <-autoplot(pca_bird.ade4)

pca_mammal.ade4 <- prcomp(trait.dist.mammals, scale. = TRUE)
mammals.pca.ade4 <- autoplot(pca_mammal.ade4)

plot_grid(anura.pca.ade4,
          squamata.pca.ade4, 
          birds.pca.ade4,
          mammals.pca.ade4, 
          align = "h",
...
tree <- prune.sample(prune.tree.amp, full_phy_amphibia)
...
```{r, squamata}
# prune sample with traits and phylogeny
traits_squamata <- full_traits_squamata
rownames(traits_squamata) <- traits_squamata[,1]
prune.tree.squa <- t(traits_squamata)
tree_squa <- prune.sample(prune.tree.squa, full_phy_squamata)

# remove
remove_squa <- c("Kinosternon_scorpioides","Paleosuchus_palpebrosus")
traits_squamata <- traits_squamata[!traits_squamata$sp %in% remove_squa,]

# check data
nrow(traits_squamata)
tree_squa

###--- handling trait data for plot
###---
head(traits_squamata[,-1])
dados <- traits_squamata[,-1]
...
for (coluna in names(dados)) {
  for (linha in seq_along(dados[, coluna])) {
    if (!is.na(dados[linha, coluna])) {
      ...
      if (coluna == "svl_mm") {
        dados[linha, coluna] <- "Body size"
      } else if (coluna == "diurnal") {
        dados[linha, coluna] <- "Circadian activity"
      } else if (coluna == "nocturnal") {
        dados[linha, coluna] <- "Circadian activity"
      } else if (coluna == "Terrestre") {
        dados[linha, coluna] <- "Habitat"
      } else if (coluna == "Arboricola") {
        dados[linha, coluna] <- "Habitat"
      } else if (coluna == "Fossorial") {
        dados[linha, coluna] <- "Habitat"
      } else if (coluna == "Aquatica") {
        dados[linha, coluna] <- "Habitat"
      }
    }
  }
...
dados
# plot
phy <- ggtree(tree_squa,layout = "circular")
...
```{r, birds}
# prune sample with traits and phylogeny
traits_birds <- full_traits_birds # copy dataset
rownames(traits_birds) <- traits_birds[,1]
prune.tree.birds <- t(traits_birds)
tree_birds <- prune.sample(prune.tree.birds, full_phy_birds)

View(full_traits_birds)
# remove
remove_birds <- c("Anodorhynchus_glaucus","Aramides_cajaneus","Cichlocolaptes_mazarbarnetti","Formicivora_acutirostris","Ortalis_squamata","Sporophila_maximiliani","Synallaxis_cinerea","Vireo_chivi","Porphyriops_melanops","Celeus_galeatus","Paraclaravis_geoffroyi","Chordeiles_nacunda","Nycticryphes_semicollaris","Eupsittula_aurea","Eupsittula_cactorum","Geranoaetus_albicaudatus","Spinus_magellanicus","Pionus_reichenowi","Hydropsalis_maculicaudus","Myiothlypis_leucophrys","Mareca_sibilatrix","Cercomacroides_laeta","Anumara_forbesi","Cyanoloxia_brissonii","Griseotyrannus_aurantioatrocristatus","Myrmoderus_loricatus","Rhopias_gularis","Stigmatura_bahiae","Pseudopipra_pipra","Ceratopipra_rubrocapilla","Philohydor_lictor","Microspingus_lateralis","Tangara_ornata","Tangara_sayaca","Systellura_longirostris","Nyctipolus_hirundinaceus","Psittacara_leucophthalmus","Rupornis_magnirostris","Sarkidiornis_sylvicola","Stephanoxis_loddigesii","Sternula_superciliaris","Formicivora_paludicola","Elaenia_sordida","Sclerurus_cearensis","Scytalopus_gonzagai" ,"Scytalopus_petrophilus","Sporophila_pileata","Lipaugus_conditus", "Xiphorhynchus_atlanticus","Microspingus_cinereus","Tangara_brasiliensis" ,"Pipraeidea_bonariensis" , "Myrmoderus_squamosus" ,"Anabacerthia_lichtensteini","Ortalis_araucuan"  ,"Orthopsittaca_manilatus","Parabuteo_leucorrhous" ,"Pseudastur_polionotus" ,"Agelaioides_fringillarius" ,"Basileuterus_auricapilla" ,"Myiothlypis_rivularis","Tangara_cyanomelas" , "Tangara_flava","Asthenes_moreirae","Automolus_lammi","Icterus_pyrrhopterus","Calidris_subruficollis","Amazilia_sapphirina" ,"Antrostomus_sericocaudatus", "Buteogallus_coronatus","Celeus_tinnunculus" ,"Synallaxis_hellmayri" ,"Herpsilochmus_scapularis","Clibanornis_rectirostris", "Sporophila_beltoni","Celeus_ochraceus", "Pygochelidon_melanoleuca","Atticora_tibialis" ,"Hirundinea_bellicosa","Serpophaga_griseicapilla","Antrostomus_rufus", "Myiothlypis_flaveola", "Myiothlypis_leucoblephara" ,"Spinus_yarrellii","Islerothraupis_cristata","Tangara_palmarum","Cantorchilus_leucotis", "Cantorchilus_longirostris","Asemospiza_fuliginosa","Microspingus_cabanisi" ,"Pheugopedius_genibarbis" , "Lipaugus_ater","Myrmoderus_ruficauda" ,"Phalcoboenus_chimango", "Psittacara_acuticaudatus" ,"Sporophila_angolensis")
traits_birds <- traits_birds[!traits_birds$sp %in% remove_birds,]
...
for (coluna in names(dados)) {
  for (linha in seq_along(dados[, coluna])) {
    if (!is.na(dados[linha, coluna])) {
      # Verificar qual coluna está sendo modificada e aplicar a substituição correspondente
      if (coluna == "BodyMass.Value") {
        dados[linha, coluna] <- "Body mass"
      } else if (coluna == "Nocturnal") {
        dados[linha, coluna] <- "Circadian activity"
      } else if (coluna == "ForStrat.watbelowsurf") {
        dados[linha, coluna] <- "Habitat"
      } else if (coluna == "ForStrat.wataroundsurf") {
        dados[linha, coluna] <- "Habitat"
      } else if (coluna == "ForStrat.ground") {
        dados[linha, coluna] <- "Habitat"
      } else if (coluna == "ForStrat.understory") {
        dados[linha, coluna] <- "Habitat"
      } else if (coluna == "ForStrat.midhigh") {
        dados[linha, coluna] <- "Habitat"
      } else if (coluna == "ForStrat.canopy") {
        dados[linha, coluna] <- "Habitat"
      } else if (coluna == "ForStrat.aerial") {
        dados[linha, coluna] <- "Habitat"
      } else if (coluna == "PelagicSpecialist") {
        dados[linha, coluna] <- "Habitat"
      }
    }
  }
...
# plot
phy <- ggtree(tree_birds,layout = "circular")+geom_tiplab(size=3)

gheatmap(phy, dados, offset=8, width=0.6, 
         colnames=FALSE, legend_title="Traits") +
  scale_fill_manual(breaks=c("Body mass","Circadian activity","Habitat"), 
                    values=c("#ffb313", "firebrick", "darkgreen"), name="Traits")
```

```{r, mammals}
# prune sample with traits and phylogeny
traits_mammals <- full_traits_mammals
rownames(traits_mammals) <- traits_mammals[,1]
prune.tree.mammals <- t(traits_mammals)
tree_mammals <- prune.sample(prune.tree.mammals, full_phy_mammals)

# remove
remove_mammals <- c("Abrawayaomys_chebezi","Abrawayaomys_ruschii","Akodon_sanctipaulensis","Brucepattersonius_griserufescens","Brucepattersonius_guarani","Brucepattersonius_misionensis","Brucepattersonius_paradisus","Cabassous_tatouay","Callicebus_barbarabrownae","Callicebus_melanochir","Callithrix_flaviceps","Dasyprocta_iacki", "Dasyprocta_prymnolopha","Dasypus_hybridus","Dasypus_septemcinctus" ,"Graomys_chacoensis" ,"Hylaeamys_oniscus", "Leontopithecus_caissara","Leontopithecus_chrysopygus","Leopardus_geoffroyi","Leopardus_guttulus" ,"Marmosa_paraguayana","Monodelphis_unistriata","Nectomys_rattus","Oxymycterus_angularis" ,"Oxymycterus_hispidus","Oxymycterus_roberti" ,"Phaenomys_ferrugineus" ,"Phyllomys_kerri" ,"Phyllomys_medius","Phyllomys_thomasi","Phyllomys_unicolor","Reithrodon_typicus","Sapajus_cay", "Sylvilagus_tapetillus","Tolypeutes_tricinctus","Trinomys_mirapitanga","Wilfredomys_oenax")
traits_mammals <- traits_mammals[!traits_mammals$sp %in% remove_mammals,]
...
for (coluna in names(dados)) {
  for (linha in seq_along(dados[, coluna])) {
    if (!is.na(dados[linha, coluna])) {
      # Verificar qual coluna está sendo modificada e aplicar a substituição correspondente
      if (coluna == "body_mass_g.avg.") {
        dados[linha, coluna] <- "Body mass"
      } else if (coluna == "cathemeral") {
        dados[linha, coluna] <- "Circadian activity"
      } else if (coluna == "crepuscular") {
        dados[linha, coluna] <- "Circadian activity"
      } else if (coluna == "diurnal") {
        dados[linha, coluna] <- "Circadian activity"
      } else if (coluna == "nocturnal") {
        ...
        dados[linha, coluna] <- "Habitat"
      } else if (coluna == "ground_dwelling") {
        dados[linha, coluna] <- "Habitat"
      } else if (coluna == "aboveground_dwelling") {
        dados[linha, coluna] <- "Habitat"
      } else if (coluna == "aquatic") {
        dados[linha, coluna] <- "Habitat"
      }
    }
  }
...
dados
# plot
phy <- ggtree(tree_mammals,layout = "circular")
...
# frogs
body_amphibia_miss <- miss.phylo.d(traits_amphibia[,-1], tree, binvar = Body_size_mm)
actv_amphibia_miss <- miss.phylo.d(traits_amphibia[,-1], tree, binvar = Noc)
habitat_amphibia_miss <- miss.phylo.d(traits_amphibia[,-1], tree, binvar = Fos)

plot(body_amphibia_miss, main='Body size')
plot(actv_amphibia_miss, main='Circadian activity')
plot(habitat_amphibia_miss, main='Habitat')

# squamata
nrow(traits_squamata)
tree_squa

body_squamata_miss <- miss.phylo.d(traits_squamata[,-1], tree_squa, binvar = svl_mm)
actv_squamata_miss <- miss.phylo.d(traits_squamata[,-1], tree_squa, binvar = diurnal)
habitat_squamata_miss <- miss.phylo.d(traits_squamata[,-1], tree_squa, binvar = Terrestre)

plot(body_squamata_miss, main='Body size')
plot(actv_squamata_miss, main='Circadian activity')
plot(habitat_squamata_miss, main='Habitat')

# birds
# Removing node labels:
tree_birds$node.label<-NULL
tree_birds <- di2multi(tree_birds)

body_birds_miss <- miss.phylo.d(traits_birds[,-1], tree_birds, binvar = BodyMass.Value)
actv_birds_miss <- miss.phylo.d(traits_birds[,-1], tree_birds, binvar = Nocturnal)
habitat_birds_miss <- miss.phylo.d(traits_birds[,-1], tree_birds, binvar =  ForStrat.watbelowsurf)

plot(body_birds_miss, main='Body mass')
plot(actv_birds_miss, main='Circadian activity')
plot(habitat_birds_miss, main='Habitat')


miss.phylo.d

# mammals
traits_mammals <- traits_mammals %>% rename(body_mass = body_mass_g.avg.)

body_mammals_miss <- miss.phylo.d(traits_mammals[,-1], tree_mammals, binvar = body_mass)
actv_mammals_miss <- miss.phylo.d(traits_mammals[,-1], tree_mammals, binvar = diurnal)
habitat_mammals_miss <- miss.phylo.d(traits_mammals[,-1], tree_mammals, binvar = fossorial)

plot(body_mammals_miss, main='Body size')
plot(actv_mammals_miss, main='Circadian activity')
plot(habitat_mammals_miss, main='Habitat')
```


##### 3.0. Save new datasets for Structural Equation Model (SEM)

Here we will consider using richness and diversification rate data that have the same number of species that were used to calculate the traits (here called harmonized data). Our sensitivity analysis showed a high correlation between these data, so there is no influence on subsequent analyses. On the other hand, the assemblage age metric uses complete datasets (here called full), since we found discrepant and uncorrelated values in our sensitivity analysis, which could change our results.

```{r}
amphibia_data <- as.data.frame(cbind(id=grid_ma$id,rich_amp=richness_amphibia$harm_rich,dr_amp= hero_dr_amphibia$diversification, aa_amp = full_aa_amphibia$mean_age_arrival, fdis_amp = FDis_metric_amphibia$FDis))

squamata_data <- as.data.frame (cbind(id=grid_ma$id,rich_squa=richness_squamata$harm_rich,dr_squa= hero_dr_squamata$diversification, aa_squa = full_aa_squamata$mean_age_arrival, fdis_squa = FDis_metric_reptiles$FDis))

birds_data <- as.data.frame(cbind(id=grid_ma$id,rich_birds=richness_birds$harm_rich,dr_birds= hero_dr_birds$diversification, aa_birds = full_aa_birds$mean_age_arrival, fdis_birds = FDis_metric_birds$FDis))

mammals_data <- as.data.frame(cbind(id=grid_ma$id,rich_mammals=richness_mammals$harm_rich,dr_mammals= hero_dr_mammals$diversification, aa_mammals = full_aa_mammals$mean_age_arrival, fdis_mammals = FDis_metric_mammals$FDis))

# join dataset
full_dataset <- left_join(amphibia_data, squamata_data, by="id")
full_dataset <- left_join(full_dataset, birds_data, by="id")
full_dataset <- left_join(full_dataset, mammals_data, by="id")

# Wed Nov  8 13:15:15 2023 ------------------------------
# adjust metrics of fdis using gawdis function
# join dataset complete with new values of fdis
full_dataset <- left_join(full_data, FDis_metric_amphibia, by="id") %>%
  rename(fdis_amp_gaw = FDis)

full_dataset <- left_join(full_dataset, FDis_metric_reptiles, by="id") %>%
  rename(fdis_rep_gaw = FDis)

full_dataset <- left_join(full_dataset, FDis_metric_birds, by="id") %>%
  rename(fdis_birds_gaw = FDis)

full_dataset <- left_join(full_dataset, FDis_metric_mammals, by="id") %>%
  rename(fdis_mammals_gaw = FDis)

### save dataset  
#setwd("E:/repos/biogeo_vertebrates-main")
write.table(full_dataset, "../03_sem/dataset_metrics.txt")
```
