...
prod_nasa <- raster("abiotic_data/MOD17A3H_Y_NPP_2022-01-01_rgb_3600x1800.TIFF")

#  NPP past from Beyer et al., 2020
file <- "../Shapefiles/LateQuaternary_Environment.nc"
tmp_raster <- brick(file, varname="npp")
# only -120.000 years
npp_past_raster <- subset(tmp_raster, "X.120000")
...
# from past -120.000
data_prod_past <- raster::extract(npp_past_raster,             
                            grid_center,   
                            fun=mean,     
                            df=TRUE)

# from nasa 
data_prod_nasa <- raster::extract(prod_nasa,             
                            grid_center,   
                            fun=mean,     
                            df=TRUE)

data_abiotic <- data.frame(clima , data_prod,
                           data_prod_past, data_prod_nasa) %>%
  rename(chelsa = "CHELSA_npp_1981.2010_V.2.1",
         past = "X.120000",
         nasa = "MOD17A3H_Y_NPP_2022.01.01_rgb_3600x1800") %>%
  select(-ID, -ID.1, -ID.2)
```

```{r, exploratory}
# check format
class(data_abiotic$past)
class(data_abiotic$nasa)
class(data_abiotic$chelsa)

cor.test(data_abiotic$past,
         data_abiotic$chelsa, method="pearson")

cor.test(data_abiotic$nasa,
         data_abiotic$chelsa, method="pearson")

cor.test(data_abiotic$past,
         data_abiotic$nasa, method="pearson")

# para o plot
cor_past_chelsa <- ggplot(data_abiotic, aes(x = scale(past), 
                  y = scale(chelsa))) +
            geom_point() +
            geom_smooth(method = "lm") +
            labs(x = "Past", y = "CHELSA")

cor_past_nasa <- ggplot(data_abiotic, aes(x = scale(past), 
                  y = scale(nasa))) +
            geom_point() +
            geom_smooth(method = "lm") +
            labs(x = "Past", y = "NASA")

cor_chelsa_nasa <- ggplot(data_abiotic, aes(x = scale(chelsa), 
                  y = scale(nasa))) +
            geom_point() +
            geom_smooth(method = "lm") +
            labs(x = "CHELSA", y = "NASA")

gridExtra::grid.arrange(cor_past_chelsa, cor_past_nasa, cor_chelsa_nasa, ncol = 3)
```

```{r}
data_analysis_map <- left_join(grid_ma, data_abiotic, by = "id")

data_analysis_map <- biscale::bi_class(data_analysis_map,
                          x = nasa,
                          y = chelsa,
                          style = "quantile",
                          dim = 3)
...
map <- ggplot() +
  geom_sf(data = data_analysis_map, mapping = aes(fill = bi_class), 
